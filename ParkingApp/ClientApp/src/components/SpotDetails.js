...
export default class SpotDetails extends Component {
  ...
  handleFormSubmission() {
    if (this.state.formChanged) {
      ...
      fetch(
        "https://localhost:44391/api/spots/edit/" + this.state.editedSpot.id,
        {
          method: "Put",
          headers: {
        ...
        .then(response => {
          if (!response.ok){
            ...
            let result = new DataResponse("Spot succesfully changed",null,true);
            this.props.onFormSubmitted(result);
        ...
        .catch(err => {
          ...
          let result = new DataResponse(null,err.message,false);
          this.props.onFormSubmitted(result); 
        });
    }
  }
  ...
  componentDidUpdate(prevProps) { 
    ...
    if (
      this.props.spot &&
      prevProps.spot &&
      this.props.spot.id !== prevProps.spot.id
    ) {
      this.setState(() => ({
        editedSpot: this.props.spot,
        newSpotLoaded: true,
  ...
  render() {
    if (this.props.spot && this.state.editedSpot) {
      return (
